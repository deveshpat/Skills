...
type: chain
sources:
  - session/systematic-debugging/SKILL.md
  - development/triage-issue/SKILL.md
...
`systematic-debugging` → `triage-issue` → `tdd`

## Entry condition

Enter at `systematic-debugging` when the error is absent or misleading — silent failures,
environment-specific issues, can't-reproduce situations. Enter at `triage-issue` when
the symptom is clear but the root cause needs codebase exploration. Enter at `tdd` directly
when root cause is already known.

## Handoff contracts

**systematic-debugging → triage-issue**
`systematic-debugging` ends with a single confirmed hypothesis: *"The failure is caused by
[exact mechanism] in [exact location], triggered when [exact condition]."* This hypothesis
is the input to `triage-issue`. If the hypothesis is still probabilistic ("probably"), do
not hand off yet — return to Step 4 of systematic-debugging.

**triage-issue → tdd**
`triage-issue` produces a GitHub issue containing: root cause analysis, a TDD fix plan
...
Both `triage-issue` and `tdd` share a durability constraint: tests and issues must describe
...
- **Hypothesizing before Q2 is verbatim** — `systematic-debugging` explicitly says: if you
...
- **Filing the issue too early** — `triage-issue` says do not ask the user to review before
